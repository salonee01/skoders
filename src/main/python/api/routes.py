...
async def match_cofounder(founder: dict):
    ...
    if not founders:
        raise HTTPException(
            status_code=404, detail="No founders available for matching"
        )

    # Convert founders into feature vectors
    founders_vectors = np.array([vectorize(f) for f in founders]).reshape(
        len(founders), -1
    )
    ...
    return {
        "matches": [
            {"name": match["name"], "match_score": round(similarities[i] * 100, 2)}
    ...
    try:
        ...
        if existing_status:
            ...
            roadmap_collection.insert_one(
                {"user_id": status.user_id, "steps": {str(status.step): status.status}}
            )
...
async def generate_fund_feasibility(prompt: dict):
    ...
    return {"generated_score": check_startup_feasibility(prompt_dict, int(prompt["funds"]))}
